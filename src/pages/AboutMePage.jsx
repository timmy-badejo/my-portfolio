...
import { FaCog, FaFilm, FaLightbulb, FaMusic } from "react-icons/fa";
...
const profileData = {
  ...
  bio:
    "I’m a visual designer and front-end designer/developer who builds clean, functional, and polished digital experiences. My work connects brand systems, interface design, and code — with a strong eye for structure, detail, and premium visual feel.",
  education: [
    {
  ...
  experience: [
    {
      ...
      skills: ["Visual Design", "Graphic Design", "Communication", "Layout Design", "Brand Consistency"],
    },
    {
      ...
      skills: ["Print Design", "Brand Systems", "Event Design", "Visual Communication", "Production Design"],
    },
    {
      ...
      skills: ["React", "UI Design", "Front-End Design", "Brand Direction", "Graphic Design"],
    },
  ],
  coreValues: [
    { title: "Creativity & Innovation", back: "I’m always pushing myself to think beyond the obvious and create work that feels memorable, useful, and visually strong." },
    { title: "User-Centered Thinking", back: "I care about how people move through a product, how they feel using it, and how design can make the experience clearer." },
    { title: "Attention to Detail", back: "Typography, spacing, alignment, colour, and small interactions matter. Those details are what make the work feel polished." },
    { title: "Lifelong Learning", back: "I keep learning through design, code, feedback, and real projects. Every project teaches me something new." },
    { title: "Resilience", back: "When something breaks or doesn’t work, I stay with it. I test, adjust, and keep building until the solution becomes clearer." },
    { title: "Collaboration", back: "I value feedback and communication. Stronger work usually comes from listening, refining, and building with others." },
  ],
  funFacts: [
    { icon: <FaCog />, title: "Car Enthusiast", description: "I’m inspired by performance cars, clean interiors, sharp body lines, and premium build quality." },
    { icon: <FaMusic />, title: "Music Lover", description: "Music helps me lock into creative flow while designing and coding." },
    { icon: <FaFilm />, title: "Anime & Movies", description: "I enjoy anime, movies, and stories with strong visuals, emotion, and world-building." },
    { icon: <FaLightbulb />, title: "Creative Systems", description: "I like turning messy ideas into structured design systems that feel clear and usable." },
  ],
};
...
export default function AboutMePage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(nameRef.current, { opacity: 0, y: -40 }, { opacity: 1, y: 0, duration: 1.2, ease: "power3.out" });
      gsap.fromTo(titleRef.current, { opacity: 0, x: -30 }, { opacity: 1, x: 0, duration: 1, ease: "power2.out", delay: 0.2 });

      [eduRefs.current, expRefs.current, coreStackRefs.current, storyRefs.current].forEach((cards) => {
  ...
  return (
    <main className="ap-about-me-page">
      ...
      <section className="ap-edu-exp-section" id="education-experience">
        ...
        <div className="ap-edu-exp-grid">
          ...
          <div className="ap-exp-stack">
            {profileData.experience.map((exp, index) => (
              <div key={exp.role} className="ap-exp-card" ref={(el) => (expRefs.current[index] = el)}>
                ...
                <p className="ap-exp-skills"><strong>Focus:</strong> {exp.skills.join(", ")}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="ap-core-values-section" id="values">
        <div className="ap-core-header">
          <h2 className="ap-core-values-title">Core Values</h2>
          <p className="ap-core-values-subtitle">The principles that steer how I design, code, communicate, and ship.</p>
        </div>

        <div className="ap-core-values-grid">
          {profileData.coreValues.map((val, index) => (
            <div key={val.title} className="ap-core-card" ref={(el) => (coreStackRefs.current[index] = el)}>
      ...
      <section className="ap-story-arc" id="story">
      ...
      <section className="ap-fun-fact-section" id="facts">
        <h2 className="ap-section-title">Fun Facts About Me</h2>

        <div className="ap-fun-fact-slider">
          <button className="ap-fun-fact-nav" onClick={() => funFactsRef.current?.scrollBy({ left: -320, behavior: "smooth" })} aria-label="Scroll fun facts left">‹</button>
          ...
          <button className="ap-fun-fact-nav" onClick={() => funFactsRef.current?.scrollBy({ left: 320, behavior: "smooth" })} aria-label="Scroll fun facts right">›</button>
        </div>
      </section>
      ...
      <footer className="ap-about-footer">
        <div>
          ...
          <p>Explore my Studio System for skills, tools, services, and process.</p>
        </div>
